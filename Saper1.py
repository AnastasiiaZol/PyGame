...
def new_game(count_bombs): #создание новой игры
    free_field=[[x,y] for y in range(0,16) for x in range(0,16)] #список свободных полей 
    ...
    massive_kletki = [[0 for i in range(stolb)] for j in range(stroki)]#матрица для хранения информации о закрытых или открытых клетках
    massive_test = [[0 for i in range(stolb)] for j in range(stroki)]#матрица для хранения информации о клетках неприлегающих к бомбам          
    finded=False
...
class Knop(pygame.sprite.Sprite):#кнопка со смайлом 
    def __init__(self):
        ...
        self.state=0
        self.win=1 #состояние проигрыша 0=проигрыш, 1=играем, 2=выигрыш
...
background = pygame.image.load("pole1.png").convert()#изображение бекграунда
    ...
    for ev in pygame.event.get():#перехват событий
        ...
        if ev.type==pygame.MOUSEMOTION:#если двигаем мышку
            if btn1.state==1 and not (btn1.rect.collidepoint(pygame.mouse.get_pos())) and btn1.win==1:#если идёт игра, курсор не попал на кнопку смайл и она была нажата
                ...
                btn1.image=pygame.image.load("btn1.png").convert()#поменять на ней картинку
            elif btn1.state==1 and not (btn1.rect.collidepoint(pygame.mouse.get_pos())) and btn1.win==2 and press==True:#она была нажата, курсор не попал на кнопку смайл, мы выиграли, левая кнопка мыши нажата
        ...
        if ev.type == pygame.MOUSEBUTTONUP:
            ...
            if ev.button == pygame.BUTTON_LEFT:#отжатие левой кнопки
                if btn1.rect.collidepoint(pygame.mouse.get_pos()):#если нажали по кнопке со смайлом и отжали
                    if btn1.state==1:
                        massive_kletki,massive,massive_test,count_bombs,timer,second=new_game(count_bombs)#начать новую игру
        ...
        if ev.type == pygame.MOUSEBUTTONDOWN:
            if ev.button == pygame.BUTTON_RIGHT:
                if btn1.win==1 and 23<pygame.mouse.get_pos()[0]<583 and 120<pygame.mouse.get_pos()[1]<680:#если идёт игра, кликнули по клетке
                    ...
                    if massive_kletki[(pygame.mouse.get_pos()[1] - 120) // 35][(pygame.mouse.get_pos()[0] - 23) // 35] ==0:#если она закрыта
                        ...
                        if state==0:#если кнопка обычная
                            gr_kletka.add(Kletka((coord[1] * 35 + 23, coord[0] * 35 + 120),"flag.png",1))#ставим флаг
                            count_bombs-=1#убовляем счётчик бомб
                        elif state==1:#если кнопка с флагом
                            gr_kletka.add(Kletka((coord[1] * 35 + 23, coord[0] * 35 + 120),"quest.png",2))#ставим вопрос
                            count_bombs+=1#прибавляем кол-во бомб
                        elif state==2:#если кнопка с вопросом
                            gr_kletka.add(Kletka((coord[1] * 35 + 23, coord[0] * 35 + 120),"kletka1.png",0))#ставим обычную клетку
                    ...
                    for kl in gr_kletka: 
                        if kl.rect.collidepoint(pygame.mouse.get_pos()):#если попали по клетке
                            ...
                            if kl.state==0: #если клетка серая 
                                massive_kletki[(pygame.mouse.get_pos()[1] - 120) // 35][(pygame.mouse.get_pos()[0] - 23) // 35] = 1#говорим, что она открыта
                                if massive[(pygame.mouse.get_pos()[1] - 120) // 35][(pygame.mouse.get_pos()[0] - 23) // 35]==0:#если стоит ноль, открываем соседние поля
                                    ...
                                    sum_flag=0                    
                                    for x in gr_kletka:#подсчёт кол-во установленных флагов
                                        if x.state==1:
                                            sum_flag+=1
                                    count_bombs=bombs-sum_flag#узнаём сколько осталось неоткрытых бомб
                                elif massive[(pygame.mouse.get_pos()[1] - 120) // 35][(pygame.mouse.get_pos()[0] - 23) // 35]==9:# если это бомба
                                    btn1.image=pygame.image.load("btn3.png").convert()
                                ...
                                if btn1.win==1:#если идёт игра
                                    ...
                                    if summa==bombs:#если кол-во закрытых клеток равняется кол-ву бомб
                                        ...
                                        timer=False#останавливаем таймер
                                        for x in gr_kletka:#на всё бомбы ставим флаги
